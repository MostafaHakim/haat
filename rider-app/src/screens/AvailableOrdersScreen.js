...
import { useSelector, useDispatch } from "react-redux";
import { orderAPI, userAPI } from "../services/api"; // <-- নিশ্চিত করো এখানে userAPI exported আছে
import {
  fetchAvailableOrdersSuccess,
  removeAvailableOrder,
  setActiveOrder,
...
import { setAvailability } from "../store/slices/authSlice";
import Icon from "react-native-vector-icons/MaterialIcons";
import { Audio } from "expo-av"; // expo-av ইনস্টল: expo install expo-av
import * as Location from "expo-location"; // expo-location ইনস্টল: expo install expo-location
...
const AvailableOrdersScreen = ({ navigation }) => {
  ...
  // --- Sound play function (optional) ---
  async function playSound() {
    try {
      console.log("Loading Sound");
      // যদি assets/notification.mp3 যোগ করো, নিচের লাইন আনকমেন্ট করো:
      // const { sound } = await Audio.Sound.createAsync(require("../assets/notification.mp3"));
      // setSound(sound);
      // await sound.playAsync();

      // যদি তুমি শুধু ডিবাগ করতে চাও, লাইনগুলো রাখো commented
    } catch (err) {
  ...
  // যখন availableOrders আসে, সাউন্ড প্লে করার চেষ্টা
  useEffect(() => {
    if (availableOrders.length > 0) {
      playSound();
      const timer = setTimeout(() => {
  ...
  // প্রথমে লোকেশন পারমিশন চাই ও লোকেশন সেট করে নাও (optional)
  useEffect(() => {
    (async () => {
      try {
  ...
  // ActiveOrder থাকলে প্রতি 30 সেকেন্ডে লোকেশন আপডেট পাঠাও
  useEffect(() => {
    let locationInterval;
    ...
    if (activeOrder && activeOrder.status !== "delivered") {
      locationInterval = setInterval(async () => {
        try {
          ...
          // backend-এ rider location update
          if (user && user._id) {
            await userAPI.updateLocation(locationData);
          }

          // local state-এ রেখে দাও
          setRiderLocation(locationData);
        } catch (error) {
          console.warn("Location update failed:", error);
        }
  ...
  const loadAvailableOrders = async () => {
    try {
      ...
      // Resilient handling: যদি response.data.data না থাকে, response.data নাও
      const ordersData = response?.data?.data ?? response?.data ?? [];
      console.log("Orders data to dispatch:", ordersData);
  ...
  }, []); // একজন বা একবার লোড হওয়ার জন্য খালি dependency array
  ...
  const renderOrderItem = ({ item }) => {
    ...
    const restaurant = item.restaurantId || {};
    const restLat =
      restaurant.location?.coordinates?.[1] ?? restaurant.location?.latitude;
    const restLon =
      restaurant.location?.coordinates?.[0] ?? restaurant.location?.longitude;

    const distance =
      user?.location && restLat !== undefined && restLon !== undefined
        ? calculateDistance(
            user.location.latitude,
            user.location.longitude,
            restLat,
  ...
  const DebugInfo = () => (
    <View style={styles.debugInfo}>
      <Text style={styles.debugText}>
        Orders: {availableOrders?.length ?? 0} | User Online:{" "}
        {user?.isAvailable ? "Yes" : "No"} | Location:{" "}
        {user?.location ? "Set" : "Not Set"}
      </Text>
    </View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          ...
          {(availableOrders?.length ?? 0) !== 1 ? "s" : ""} available nearby
        </Text>
